...
late  final CategoryModel categoryModel;
  ...
  Widget build(BuildContext context) {
    ...
    if(_isCategory) {
      Get.find<RestaurantController>().getCategoryList(Product());
    }else {
      Get.find<RestaurantController>().getSubCategoryList(categoryModel.id, Product());
    }
    ...
    return Scaffold(
      ...
      body: GetBuilder<RestaurantController>(builder: (restController) {
        ...
        return _categories!=null ? _categories.length > 0 ? RefreshIndicator(
          onRefresh: () async {
            if(_isCategory) {
              await Get.find<RestaurantController>().getCategoryList(Product());
            }else {
              await Get.find<RestaurantController>().getSubCategoryList(categoryModel.id!, Product());
            }
          },
                ...
                child: Container(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(_categories[index].name.toString(), style: robotoRegular, maxLines: 1, overflow: TextOverflow.ellipsis),
